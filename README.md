...
- 视频：支持 MP4、MOV、AVI、MKV、WEBM、M4V 输入；默认输出 MP4 全帧水印文件，增强对剪辑、截取片段的保留能力。
- 网页：支持添加水印和读取验证水印。
...
视频水印使用“全帧快速 DCT 认证水印”。默认输出为 MP4，便于在常见设备和播放器中直接打开；程序会给每一帧写入短认证包，读取时抽帧采样并命中即停，兼顾抗剪辑能力和读取速度。
